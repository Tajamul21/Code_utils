...
def save_preds(data_path, args, split):
    ...
    for i, image_path in enumerate(tqdm(image_paths)):
        ...
        pred_data = torch.cat((output['boxes'], output['scores']), 1).numpy()
        
        # pred_data_path = image_path.replace(f"/{split}/", f"/{split}_focalnet/")[:-4]+"_preds.txt"
        pred_data_path = os.path.join(f"{data_path}"+"_focalnet/",image_path.split("/")[-1][:-4]+"_preds.txt")
...
if __name__ == '__main__':
    ...
    ROOT = "/home/kshitiz/scratch/FocalNet-DINO/exps/prev_baselines/aiims_4k/"
    ...
    model_checkpoint_path = os.path.join(ROOT,"checkpoint00{}.pth".format(checkpoint_number))
    ...
    DATA_PATH = "/home/kshitiz/scratch/FocalNet-DINO/MULTI_MODEL_DATA/IRCH_DATA/Mammo_PNG"
    # save_preds(DATA_PATH, args, "Train")
